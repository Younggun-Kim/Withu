...
class JobPostingRepositoryImpl implements JobPostingRepository {
  ...
  JobPostingRepositoryImpl({required this.jobPostingApi});
  ...
  Future<ApiResponse<JobPostingsDto>> search({
    ...
    return await jobPostingApi.search(status: status, page: page);
  }
  ...
  Future<ApiResponse<JobPostingDetailDto>> update({
    ...
    return await jobPostingApi.update(id: id, requestData: dto);
  }

  /// 공고 상세 조회
  @override
  Future<ApiResponse<JobPostingDetailDto>> get({required String id}) async {
    return await jobPostingApi.get(id: id);
  }

  /// 공고 마감
  @override
  Future<ApiResponse<JobPostingDetailDto>> close({required String id}) async {
    return await jobPostingApi.close(id: id);
  }

  /// 공고 삭제
  @override
  Future<ApiResponse<DeleteResponseDto>> delete({required String id}) async {
