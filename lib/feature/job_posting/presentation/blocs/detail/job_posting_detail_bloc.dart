...
class JobPostingDetailBloc
  ...
  void _onGettingDetailData(
    ...
    await _fetchData(jobPostingId: event.id, emit: emit);
  }
  ...
  void _onClosedJobPosting(
    ...
    result.when(
      success: (JobPostingDetailEntity data) {
        emit(state.copyWith(status: JobPostingDetailStatus.close));
      },
      fail: (String message) {
        emit(
          state.copyWith(
  ...
  void _onDeletedJobPosting(
    ...
    final Either<bool> result = await useCase.delete(id: jobPostingId);

    result.when(
      success: (bool data) {
        emit(state.copyWith(status: JobPostingDetailStatus.delete));
      },
      fail: (String message) {
        emit(
          state.copyWith(
  ...
  void _onRefreshed(
    ...
    await _fetchData(jobPostingId: jobPostingId, emit: emit);
  }
  ...
  Future _fetchData({
    ...
    result.when(
      success: (JobPostingDetailEntity data) {
        emit(
          state.copyWith(status: JobPostingDetailStatus.success, entity: data),
        );
      },
      fail: (String message) {
        emit(
