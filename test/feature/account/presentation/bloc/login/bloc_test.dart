...
void main() {
  group(LoginBloc, () {
      ...
      expect:
          () => [
            isA<LoginState>()
                ...
                .having((state) => state.loginId.isValid, 'isValidId', true)
                .having(
      ...
      expect:
          () => [
            isA<LoginState>()
                ...
                .having((state) => state.loginId.isValid, 'isValid', false)
                .having(
                  (state) => state.isEnabledLogin,
                  'isEnabledLogin',
                  false, // password가 아직 없으므로 false,
    ...
    blocTest(
      ...
      // Given
      setUp: () {
        when(() => loginUseCase.login(entity: any(named: 'entity'))).thenAnswer(
          (_) async => LoginResultEntity(isLoggedIn: true, message: '로그인 성공'),
        );
      },
      // When
      ...
      expect:
          () => [
            ...
            isA<LoginState>()
                ...
                .having((state) => state.message, 'message', '로그인 성공'),
          ],
      verify: (_) {
        verify(
    ...
    blocTest(
      ...
      setUp: () {
        when(() => loginUseCase.login(entity: any(named: 'entity'))).thenAnswer(
          (_) async =>
              LoginResultEntity(isLoggedIn: false, message: '중복된 아이디 입니다.'),
        );
      },
      // When
      act: (bloc) => bloc.add(LoginBtnPressed()),
